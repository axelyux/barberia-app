...
// Solo mira el estado (una columna) en vez de traer el tenant completo — se llama en
// cada mensaje entrante para decidir si el bot debe contestar o quedarse callado.
export const getTenantStatus = async (tenantId) => {
    const id = await resolveTenantId(tenantId)
    const tenant = await prisma.tenant.findUnique({ where: { id }, select: { status: true } })
    return tenant?.status ?? 'PAUSED'
}
...
// Solo dígitos, y recorta el prefijo de código de país de México (52 / 521) para que
// "5218331234567" y "8331234567" se traten como el mismo cliente en vez de crear dos
// filas de Customer distintas por un formato de número diferente.
...
// Busca (o crea) el Customer de esta barbería para un teléfono dado. Se usa al agendar
// desde WhatsApp para que Booking.customerId deje de quedar siempre en null — sin esto,
// el historial de cliente y el CRM del panel nunca veían las citas agendadas por el bot.
export const findOrCreateCustomer = async (tenantId, phone, name) => {
    const id = await resolveTenantId(tenantId)
    const normalized = normalizePhone(phone)
    const existing = await prisma.customer.findUnique({ where: { tenantId_phone: { tenantId: id, phone: normalized } } })
    if (existing) return existing
    return prisma.customer.create({
        data: { tenantId: id, phone: normalized, name: name?.trim() || normalized },
    })
}

export const saveBooking = async (
    { customerPhone, customerName, serviceId, barberId, durationMin, priceChargedCents, scheduledAt, day, time },
    tenantId
) => {
    const id = await resolveTenantId(tenantId)
    const customer = await findOrCreateCustomer(id, customerPhone, customerName)
...
// --- Estado de conversación persistente (ver whatsapp-meta-bot.js) ---
// Se guarda en cada paso del flujo de agendado para no perder el avance del cliente si
// el proceso del bot se reinicia. Filas de más de 2 horas se consideran vencidas y se
// ignoran (el cliente simplemente empieza de nuevo si escribe después de ese margen).
const CONVERSATION_TTL_MS = 2 * 60 * 60 * 1000

export const saveConversationStep = async (tenantId, phone, step, data) => {
    const id = await resolveTenantId(tenantId)
    const normalized = normalizePhone(phone)
    await prisma.conversationState.upsert({
        where: { tenantId_phone: { tenantId: id, phone: normalized } },
        update: { step, data },
        create: { tenantId: id, phone: normalized, step, data },
    })
}

export const getConversationState = async (tenantId, phone) => {
    const id = await resolveTenantId(tenantId)
    const normalized = normalizePhone(phone)
    const row = await prisma.conversationState.findUnique({ where: { tenantId_phone: { tenantId: id, phone: normalized } } })
    if (!row) return null
    if (Date.now() - row.updatedAt.getTime() > CONVERSATION_TTL_MS) return null
    return row
}

export const clearConversationState = async (tenantId, phone) => {
    const id = await resolveTenantId(tenantId)
    const normalized = normalizePhone(phone)
    await prisma.conversationState.deleteMany({ where: { tenantId: id, phone: normalized } })
}
