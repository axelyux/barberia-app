...
export function parseTimeText(text) {
    ...
    // Las listas de horarios mandan un id exacto ("time_2130" = 21:30), sin ambigüedad.
    const idMatch = String(text).trim().match(/^time_(\d{2})(\d{2})$/)
    if (idMatch) {
        return { hour: parseInt(idMatch[1], 10), minute: parseInt(idMatch[2], 10), explicitMeridiem: true }
    }

    // Normaliza "p. m." / "a. m." (formato que devuelven los botones de WhatsApp y
    // el formateo es-MX) a "pm" / "am", que es lo que entiende la expresión de abajo.
    const clean = stripAccents(text)
        .replace('a las', '')
        .replace(/([ap])\.\s*m\.?/g, '$1m')
        .trim()
    // Acepta el separador de minutos como ":", ".", " " o pegado ("10:30", "10.30", "10 30", "1030").
    ...
    // explicitMeridiem indica si el cliente sí dijo am/pm. Cuando no lo dijo, quien llama
    // puede corregir una hora que ya pasó (ej. "9:00" a las 8 de la noche = 9 pm, no 9 am).
    return { hour, minute, explicitMeridiem: isPM || isAM }
}
...
// Elige un barbero de una lista por id de lista interactiva ("brb_<id>" o "brb_any"),
// número de menú (la última opción siempre es "cualquiera"), nombre, o la palabra
// "cualquiera". Devuelve: un barbero (objeto), null (sin preferencia, "cualquiera"), o
// undefined (no se entendió, hay que volver a preguntar) — a propósito null !== undefined
// aquí, para distinguir "no le importa" de "no supimos qué contestó".
export function matchBarberChoice(text, barbers) {
    if (!text) return undefined
    const clean = stripAccents(text).trim()

    if (clean.startsWith('brb_')) {
        const id = text.trim().slice(4)
        if (id === 'any') return null
        return barbers.find((b) => b.id === id) ?? undefined
    }
    if (clean.includes('cualquier')) return null

    const asNumber = parseInt(clean, 10)
    if (!Number.isNaN(asNumber)) {
        if (asNumber === barbers.length + 1) return null // última opción del menú = "cualquiera"
        if (barbers[asNumber - 1]) return barbers[asNumber - 1]
    }

    return barbers.find((b) => nameMentioned(text, b.name)) ?? undefined
}
...
export function matchServiceChoice(text, services) {
    ...
    // Las listas interactivas de WhatsApp devuelven el id de la fila elegida, no su texto.
    // Usamos el prefijo "svc_" para identificar el servicio sin ambigüedad.
